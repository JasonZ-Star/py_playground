...
- 30 秒执行超时保护，运行输出展示
- 本地代码自动保存与链接分享（`?code=...`/`?template=...`/长链 LZString 跨设备分享/短链 localStorage 7 天同浏览器）
...
分享功能支持两种链接模式，**默认走 LZString 长链（跨设备/跨浏览器可用）**，并提供短链切换：

| 模式 | URL 示例 | 跨设备 | 长度 | 有效期 |
|------|----------|--------|------|--------|
| **长链（默认）** | `http://host/s/<LZString压缩>` | ✅ 任意设备/浏览器 | 较大（与代码成正比） | 永久 |
| **短链（备选）** | `http://host/s/<12位ID>` | ❌ 仅同浏览器 | 短（约 30 字符） | 7 天 |
| **兜底（兼容）** | `http://host/?code=<base64>` | ✅ 任意设备 | 中等（base64 膨胀） | 永久 |

**安全阈值**：长链超过 7800 字符时自动回退到短链（浏览器 URL 长度上限 8KB）。

Modal 中包含"长链 · 跨设备 / 短链 · 同浏览器"切换按钮组，一键复制并切换类型。
